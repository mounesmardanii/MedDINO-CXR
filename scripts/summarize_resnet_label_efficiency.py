...
def reconstruct_training_state(history):
    best = -math.inf
    selected_epoch = None
    ...
    for index, record in enumerate(history):
        ...
        if (
            ...
            raise RuntimeError(
                f"Training should have stopped at epoch {epoch}"
            )
...
def load_subset_size(seed, fraction):
    ...
    require(
        archive_path.exists(),
        f"Missing subset archive: {archive_path}",
    )
    ...
    with np.load(archive_path) as archive:
        require(
            ...
            f"Missing subset key {key}: {archive_path}",
        )
...
def audit_run(seed, fraction):
    ...
    require(
        ...
        f"Invalid history: {history_path}",
    )
    ...
    require(
        ...
        f"Epoch limit exceeded: fraction={fraction}, seed={seed}",
    )
    ...
    for metric in SCALAR_METRICS:
        require(
            metric in metrics,
            f"Missing metric {metric}: {checkpoint_path}",
        )

        require(
            metric in selected_record,
            f"Missing history metric {metric}",
        )

        require(
            values_match(
                metrics[metric],
                selected_record[metric],
            ),
            f"Checkpoint/history mismatch for {metric}: fraction={fraction}, seed={seed}",
        )

    per_class_roc = metrics[
        "per_class_roc_auc"
    ]

    per_class_ap = metrics[
    ...
    (
        reconstructed_epoch,
        reconstructed_score,
        final_no_improvement,
    ) = reconstruct_training_state(history)
    ...
    if stopped_epoch < MAX_EPOCHS:
        require(
            final_no_improvement >= PATIENCE,
            f"Invalid early stopping: fraction={fraction}, seed={seed}",
        )

        termination = "early_stopping"
    else:
        termination = "max_epochs"

    row = {
        "model": "resnet18",
        "fraction_percent": fraction,
        "seed": seed,
        "train_samples": load_subset_size(
            seed,
            fraction,
        ),
    ...
    for index, class_name in enumerate(
        ...
        row[
            f"{class_name}_average_precision"
        ] = float(
            per_class_ap[index]
        )
...
def build_summary(runs):
    ...
    for fraction in FRACTIONS:
        group = runs[
            runs["fraction_percent"] == fraction
        ]
        ...
        require(
            group["train_samples"].nunique() == 1,
            f"Train sample mismatch for fraction {fraction}",
        )
        ...
        row = {
            ...
            "selected_epoch_mean": float(
                group["selected_epoch"].mean()
            ),
            "selected_epoch_std": float(
                group["selected_epoch"].std(
                    ddof=1
                )
            ),
            "stopped_epoch_mean": float(
                group["stopped_epoch"].mean()
            ),
            "stopped_epoch_std": float(
                group["stopped_epoch"].std(
                    ddof=1
                )
            ),
        }
